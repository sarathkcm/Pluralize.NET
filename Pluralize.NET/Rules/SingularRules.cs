        ...
        public static IList<ReplaceRule> GetRules()
        {
            return new List<ReplaceRule>
            {
               // rules are ordered more generic first
                ...
                new ReplaceRule { Condition = new Regex("(x|ch|ss|sh|zz|tto|go|cho|alias|[^aou]us|t[lm]as|gas|(?:her|at|gr)o|ris)(?:es)?$",RegexOptions.IgnoreCase), ReplaceWith = "$1"},
                new ReplaceRule { Condition = new Regex("(analy|ba|diagno|parenthe|progno|synop|the|empha|cri)(?:sis|ses)$",RegexOptions.IgnoreCase), ReplaceWith = "$1sis"},
